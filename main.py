"""
SAVAN Certificate Render Microservice — FastAPI on Railway
...
ASSETS_PATH = Path(__file__).parent / "persistent_assets.json"
...
def get_assets() -> dict:
    ...
    if _assets is None:
        if not ASSETS_PATH.exists():
            raise RuntimeError(f"Assets not found: {ASSETS_PATH}")
...
def sig_tag(b64,sw,sh,cx,ytop,w=2970):
    ...
    return f'<image x="{x}" y="{ytop}" width="{w}" height="{h}" preserveAspectRatio="xMidYMid meet" xlink:href="data:image/png;base64,{b64}"/>'
...
def build_base_svg(assets,fonts,template,sponsored_by,
    ...
    if t2:
        ...
        if collab_logo_bytes:
            ...
            svg=svg.replace('</svg>',f'<circle cx="{cx}" cy="{cy}" r="{r}" fill="#FFFFCC"/>'
...
def render_one(base_svg,assets,fonts,name,year,month,session,seq,date_str,template,photo_bytes):
...
async def render_endpoint(
    ...
    try:
        assets=get_assets(); fonts=get_fonts(assets)
    ...
    for i,part in enumerate(participants):
        ...
        try:
            ...
            results.append({"cert_id":cert_id,"name":part["name"],"date":part["date"],"pdf_base64":pdf_b64})
        except Exception as e:
